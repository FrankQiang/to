...
!function($) {
            ...
            $.each(this.options.buttons, function(index, ele) {
                if (ele=='|') {
                    ...
                    if (ele=='heading') {
                        ...
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading1"># 一级标题</a></li>');
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading2">## 二级标题</a></li>');
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading3">### 三级标题</a></li>');
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading4">#### 四级标题</a></li>');
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading5">##### 五级标题</a></li>');
                        L.push('<li><a href="javascript:void(0)" data-type="md" data-cmd="heading6">###### 六级标题</a></li>');
                        L.push('</ul>');
                    }
                    else if(ele=='image'){
                        L.push('<button data-toggle="modal" data-target="#image-modal" type="button" data-type="md" data-cmd="' + ele + '" title="' + $tooltip + '" class="btn' + ($icon.indexOf('icon-white')>=0 ? ' btn-info' : '') + '"><i class="' + $icon + '"></i></button>');
                    }
                    else{
    ...
    var commands = {
        ...
        link: function(delegate) {
            var s = '<div data-backdrop="static" class="modal hide fade"><div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button><h3>设置链接</h3></div>'
                  + '<div class="modal-body"><form class="form-horizontal"><div class="control-group"><label class="control-label">链接名:</label><div class="controls"><input name="text" type="text" value="" placeholder="例 : 百度"/></div></div>'
                  + '<div class="control-group"><label class="control-label">网址:</label><div class="controls"><input name="link" type="text" placeholder="例 : http://www.baidu.com" /></div></div>'
                  + '</form></div><div class="modal-footer"><a href="#" class="btn btn-primary">设置</a><a href="#" class="btn" data-dismiss="modal">取消</a></div></div>';
            $('body').prepend(s);
        ...
        image: function(delegate) {
            ...
            var s = '<div data-backdrop="static" class="modal hide fade"><div class="modal-header"><button type="button" class="close">&times;</button><h3>​插入图片</h3></div>'
                  + '<div class="modal-body"><div style="width:530px;"><div class="alert alert-error hide"></div><div class="row">'
                  + '<div class="span" style="width:230px"><div>预览:</div><div class="preview" style="width:200px;height:150px;border:solid 1px #ccc;padding:4px;margin-top:5px;background-repeat:no-repeat;background-position:center center;background-size:cover;"></div></div>'
                  + '<div class="span" style="width:300px"><form>'
                  + '<input name="text" type="hidden" value="" />'
                  + '<label>文件:</label><input name="file" type="file" style="margin-bottom:25px"/>'
                  + '<label>进度:</label><div class="progress progress-striped active" style="width:220px; margin-top:6px;margin-bottom:6px"><div class="bar" style="width:0%;"></div></div>'
                  + '</form></div>'
                  + '</div></div></div><div class="modal-footer"><button class="btn btn-primary">上传</button> <button class="btn btn-cancel">取消</button></div></div>';
            $('body').prepend(s);
            var $modal = $('body').children(':first');
            ...
            $modal.find('.btn-primary').click(function() {
                ...
                var f = $file.val();
                if (!f) {
                    $alert.text('选择图片').addClass('alert-error').show();
                    return;
                }
                ...
                try {
                    ...
                    xhr.addEventListener('load', function(evt ) {
                        var rurl = evt.target.responseText
                        var tomatch = /^http:\/\/www\.wocycle\.com\/uploads\/users\/blog\/\S{1,}\.jpeg|png|jpg$/i;
                        if (!tomatch.test(rurl)) {
                            $alert.addClass('alert-error').text('上传失败!').show();
                            $status.uploading = false;
                        }
                        else {
                            delegate.unselect();
                            var s = '\n![' + text.replace('[', '').replace(']', '') + '](' + rurl + ')\n';
                            delegate.paste(s);
    ...
    $.fn.markdown.defaults = {
        ...
        upload_image_url: 'http://www.wocycle.com/image/upload',
        upload_file_url: '',
    };
